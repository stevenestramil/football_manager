...
async def get_players(
    ...
    return await service.get_players(team_id=team_id, position=position, min_age=min_age)


@router.get("/{player_id}", response_model=PlayerRead)
async def get_player(player_id: int, service: PlayerServiceDep):
    player = await service.get_player(player_id=player_id)
    if not player:
...
async def add_player_to_team(
    ...
    try:
        return await service.assign_player_to_team(
            player_id=player_id, team_id=team_assign.team_id
        )
...
async def remove_player_from_team(player_id: int, service: PlayerServiceDep):
    try:
        await service.remove_player_from_team(player_id)
    except UnknownPlayerError:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Player not found")
